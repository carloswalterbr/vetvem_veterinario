...
angular.module('ui.rCalendar', ['ui.rCalendar.tpls'])
    .constant('calendarConfig', {
        formatDay: 'dd',
        formatDayHeader: 'EEE',
        formatDayTitle: 'dd MMMM yyyy', //'MMMM dd, yyyy', 
        formatWeekTitle: 'MMMM yyyy (w)', //'MMMM yyyy, Week w',
    ...
    .directive('monthview', ['dateFilter', function (dateFilter) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                scope.select = function (selectedDate, events) { // mes
					//alert('clicou no dia na view mes - aciona scope.select (line 456):'+selectedDate);
    ...
    .directive('weekview', ['dateFilter', '$rootScope', function (dateFilter, $rootScope) {
        'use strict';
        return {
            restrict: 'EA',
            replace: true,
...
// <----------------------------------- <---- <---- 
                scope.cw = function(time, events) {
					// só broadcasta se não houver evento nesta datetime 
					//	 se houver, trate via callback $scope.onEventSelected no app.Controller
					if (!events) { // antes da carga inicial do $scope.calendar.eventSource o evento não existe
						scope.$emit('marca-desmarca', {args: {hora:time}});	
						//alert('não tem evento');
					}
					else {
						if (events.length ==0) { // após a carg do $scope.calendar.eventSource ele existe porém vazio
							scope.$emit('marca-desmarca', {args: {hora:time}});	
						}
					}
				} 
// <----------------------------------- <---- <----
				...
				scope.select = function (selectedTime, events) { // week
					alert(selectedDate);
                    if (scope.timeSelected) {
    ...
    .directive('dayview', ['dateFilter', function (dateFilter) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                scope.select = function (selectedTime, events) { // day
					alert(selectedDate);
                    if (scope.timeSelected) {
                        scope.timeSelected({selectedTime: selectedTime, events: events});
                    }
...
// o style abaixo foi incluído para poder pintar o evento dinamicamente através do campo 'cor' de calendar.eventSource
angular.module("templates/rcalendar/displayEvent.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("templates/rcalendar/displayEvent.html",
    "<div class=\"calendar-event-inner\" style=\"font-size:3vmin;background-color:#{{displayEvent.event.cor}}\">{{displayEvent.event.title}}</div>");
}]);
...
angular.module("templates/rcalendar/monthviewEventDetail.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("templates/rcalendar/monthviewEventDetail.html",
    ...
    "        <tr ng-repeat=\"event in selectedDate.events         | filter:'x7'          \" ng-click=\"eventSelected({event:event})\">\n" +
...
angular.module("templates/rcalendar/week.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("templates/rcalendar/week.html",
    ...
    "                            </td>\n" +
    "                            <td ng-repeat=\"tm in row track by tm.time\" class=\"calendar-cell\" ng-click=\"cw(tm.time, tm.events)\">\n" +
    "                                <div ng-class=\"{'calendar-event-wrap': tm.events}\" ng-if=\"tm.events\">\n" +
    "                                    <div ng-repeat=\"displayEvent in tm.events                     \" class=\"calendar-event\"\n" +
    "                                         ng-click=\"eventSelected({event:displayEvent.event})\"\n" +
    "                                         ng-style=\"{top: (37*displayEvent.startOffset/hourParts)+'px',left: 100/displayEvent.overlapNumber*displayEvent.position+'%', width: 100/displayEvent.overlapNumber+'%', height: 37*(displayEvent.endIndex -displayEvent.startIndex - (displayEvent.endOffset + displayEvent.startOffset)/hourParts)+'px'}\"\n" +
